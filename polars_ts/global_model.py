...
class GlobalForecaster:
    ...
    def fit(self, df: pl.DataFrame) -> GlobalForecaster:
        ...
        # Track columns generated by feature engineering
        ...
        # Determine feature columns: generated + future covariates +
        # ID-encoded + static-encoded columns
        id_static_cols = set(featured.columns) - set(list(original_cols) + list(generated) + list(future_cov_set))
        # Remove raw static feature columns from that set
        if self.static_features:
            id_static_cols -= set(self.static_features)
        self.feature_columns_ = [
            c for c in featured.columns if c in generated or c in future_cov_set or c in id_static_cols
        ]
    ...
    def predict(
        ...
        for group_id, group_df in sorted_df.group_by(self.id_col, maintain_order=True):
            ...
            if self.past_covariates:
                pcov_buffers = {col: group_df[col].to_list() for col in self.past_covariates}
